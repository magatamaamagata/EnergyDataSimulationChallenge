...
class Calculator
  ...
  def calculate
    ...
    @prices=[]
    # 条件から各社料金を計算
    arrays_all.map.with_index do |array,i|
      # 配列の中身の情報
      if array[:contract_current].include?(@ampare)
        # 基本料金を定義 アンペアがbasic_chargeにあればその基本料金
        ...
        # 単価決定の条件式1 
        # range1= array[:range][0]
        # range2= array[:range][1]
        # pay_per_use1 =array[:pay_per_use][0]
        # pay_per_use2 =array[:pay_per_use][1]
        # pay_per_use3 =array[:pay_per_use][2]
      ...
      if sum != nil
        @prices << { provider_name: "#{arrays_all[i][:provider_name]}", plan_name: "#{arrays_all[i][:plan_name]}", price: sum }
      end
    end

    # 料金の計算結果の個数だけ会社とプラント料金を、配列@resultsに入れる。
    # @prices= @prices.map.with_index do |price, i|
    #   if price != nil
    #   { provider_name: "#{arrays_all[i][:provider_name]}", plan_name: "#{arrays_all[i][:plan_name]}", price: price }
    #   end
    # end
  end
  # 結果ゲッター
